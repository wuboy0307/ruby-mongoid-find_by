...
describe Mongoid::Finders::FindBy do
  ...
  let :clean_model do
    Object.send(:const_set,
      obj = Mongoid::SpecHelpers.random_class, Class.new(model))
  ...
  context "after include" do
    describe "Model.method_missing" do
      it "should call super if the method doesn't start with find_by" do
        expect1 = expect { model.send(Mongoid::SpecHelpers.random(10)) }
        expect2 = expect { model.send(Mongoid::SpecHelpers.random(10)) }
        expect3 = expect { model.send(Mongoid::SpecHelpers.random(10)) }

        [expect1, expect2, expect3].each do |expect_thee|
          expect_thee.to raise_error NoMethodError
        end
      end

      it "should define find_by if the field exists" do
      ...
      it "should not define find_by if the field does not exist" do
      ...
      it "should define find_by with unlimited _and_ if all fields exist" do
      ...
      it "should not define find_by with unlimited _and_ all don't exist" do
      ...
      it "should define find_all_by if the field exists" do
      ...
      it "should define find_all_by with unlimited _and_ if all exist" do
      ...
      it "shouldn't define find_all with unlimited _and_ if all don't exist" do
        expect1 = expect { model.find_all_by_f1_and_f4(1, 4) }
        expect2 = expect { model.find_all_by_f2_and_f4(2, 4) }
        expect3 = expect { model.find_all_by_f3_and_f4(3, 4) }
